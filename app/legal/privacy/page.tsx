  ...
  return (
    ...
    >
      <p style={legalStyles.lead}>
        株式会社アウトインプラス（以下「当社」といいます。）は、「JLPT PASS N3・N2」（以下「本サービス」といいます。）において取得する個人情報および利用情報を、以下の方針に基づき取り扱います。
      </p>
      ...
      <LegalSection title="2. 利用目的">
        <ul style={legalStyles.ul}>
          ...
          <li>決済処理事業者を通じた決済確認、入金確認、購入履歴管理、返金判断を行うため</li>
      ...
      <LegalSection title="3. 外部サービスの利用">
        <p style={legalStyles.p}>本サービスでは、認証・データ保存、決済、AI会話・AIスピーキング等の提供に外部サービスを利用する場合があります。</p>
        <p style={legalStyles.p}>クレジットカード情報は当社では保持せず、外部の決済サービス上で処理されます。外部サービスの利用に際し、サービス提供に必要な範囲で情報が送信・保存・処理される場合があります。</p>
      </LegalSection>
