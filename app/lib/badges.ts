...
export type BadgeGroup = "battle" | "study" | "ai" | "streak" | "score" | "secret"
...
type UnlockState = {
  ...
  maxScore?: number
  streak?: number
  industryCounts?: Partial<Record<string, number>>
}
...
const battlePlayBadges = buildSeries("battle", "battle-play", "🔥", "ゲーム", "ゲームプレイ達成", "ゲームを合計{n}回プレイ", [3, 5, 10, 20, 30, 50, 75, 100, 150, 200, 300, 500], 1000)
const attackPlayBadges = buildSeries("battle", "attack-play", "🏁", "アタック", "アタック挑戦達成", "アタックを合計{n}回プレイ", [1, 3, 5, 10, 20, 30, 50, 75, 100], 1100)
const tileDropBadges = buildSeries("battle", "tile-drop-clear", "🧩", "文字ブレイク", "文字ブレイク達成", "文字ブレイクを合計{n}回クリア", [1, 3, 5, 10, 20, 30, 50, 75, 100], 1200)
const flashJudgeBadges = buildSeries("battle", "flash-judge-clear", "⚡", "瞬判ジャッジ", "瞬判ジャッジ達成", "瞬判ジャッジを合計{n}回クリア", [1, 3, 5, 10, 20, 30, 50, 75, 100], 1300)
const memoryBurstBadges = buildSeries("battle", "memory-burst-clear", "🧠", "フラッシュ記憶", "フラッシュ記憶達成", "フラッシュ記憶を合計{n}回クリア", [1, 3, 5, 10, 20, 30, 50, 75, 100], 1400)
const studyQuestionBadges = buildSeries("study", "study-questions", "📚", "学習", "累計回答達成", "累計{n}問解く", [1, 5, 10, 20, 30, 50, 75, 100, 150, 200, 300, 500, 750, 1000, 1500, 3000, 5000], 2000)
const scoreBadges = buildSeries("score", "score", "🥇", "高得点", "高得点達成", "どれかの教材で{n}点以上を取る", [50, 60, 70, 80, 90, 95, 100], 2600)
const streakBadges = buildSeries("streak", "streak", "🌱", "継続", "継続達成", "{n}日連続で学習する", [2, 3, 5, 7, 10, 14, 21, 30, 60, 100, 180, 365], 2800)
...
export function getBadgeMeta(badgeId: string): BadgeDef {
  ...
  return STATIC_CATALOG.find((b) => b.id === badgeId) ?? {
    id: badgeId,
    icon: "🏅",
...
export function getRarityColors(rarity: BadgeRarity) {
  switch (rarity) {
    case "common": return { border: "#d1d5db", bg: "linear-gradient(135deg, #f9fafb 0%, #eef2f7 100%)", glow: "rgba(107,114,128,0.18)" }
    case "rare": return { border: "#93c5fd", bg: "linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)", glow: "rgba(59,130,246,0.20)" }
    case "epic": return { border: "#c4b5fd", bg: "linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%)", glow: "rgba(124,58,237,0.22)" }
    case "legend": return { border: "#fcd34d", bg: "linear-gradient(135deg, #fff7ed 0%, #fef3c7 100%)", glow: "rgba(245,158,11,0.22)" }
  }
}
