...
export default function LegalFooter({ compact = false }: Props) {
  return (
    <footer style={{ marginTop: compact ? 20 : 32, padding: compact ? "14px 0 8px" : "20px 0 10px", borderTop: "1px solid rgba(17,24,39,.08)" }}>
      ...
      <p style={{ marginTop: 12, fontSize: 12, lineHeight: 1.7, color: "rgba(17,24,39,.68)" }}>
        本サービスは JLPT PASS N3・N2 Japanese Study App です。カード情報は当アプリでは保持せず、KOMOJU Checkout上で処理されます。
      </p>
    </footer>
  )
}
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return <Link href={href} style={{ fontSize: 13, fontWeight: 800, color: "#2563eb", textDecoration: "none" }}>{children}</Link>
}
